...
class AStackLattice(ABCPrinter):
    ...
    # an associated StackGeometryParser subclass
    sgp = None
    ...
    def inverted(self):
        stk = type(self).from_dimensions(self.scale, self.D, self.L)
        stk.invert()
        return stk
...
class ATwinStackLattice(AStackLattice):
    ...
    def get_points(self) -> np.ndarray:
        ...
        for i in range(self._nz):
            ...
            for t in self.basis:
                for bpt in self.basis[t]:
                    stk_nz = self.supercell.nz
                    if (i % (2 * stk_nz) > stk_nz) and t > 1:
    ...
    def inverted(self):
        stk = type(self).from_dimensions(self.scale, self.D, self.L, self.P)
        stk.invert()
        return stk


class APaddedStackLattice(AStackLattice):
    @classmethod
    def get_supercell(cls, *args):
        raise ValueError("generally non-periodic padded stack structure is its "
                         "own supercell")
    ...
    @property
    def supercell(self):
        # generally non-periodic padded stack structure is its own supercell
        return self

    def __init__(self, stk_bottom, stk_core, stk_top,
                 nz_bottom, nz_core, nz_top,
                 vz_unit=None):

        planes = []
        vr = np.zeros((nz_bottom + nz_core + nz_top, 3))

        for i in range(nz_bottom):
            planes.append(stk_bottom.planes[-i])
            vr[i] = stk_bottom.vr[-i]

        for i in range(nz_core):
            planes.append(stk_core.planes[i])
            vr[i + nz_bottom] = stk_core.vr[i]

        for i in range(nz_top):
            planes.append(stk_top.planes[i])
            vr[i + nz_bottom + nz_core] = stk_top.vr[i]

        if vz_unit is None:
            vz_unit = stk_core.vz_unit
