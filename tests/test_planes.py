...
class WriteAllTest(PlaneObjectsTest):
    def setUp(self):
        self.plane_kwargs = dict(
            scale=5.65,
            width=50.,
            m_xy=0
        )
    ...
    def test_write_all_planes(self):
        for plane in self.plane_types:
            if 'm_xy' in self.argspecs[plane]:
                p = plane(**self.plane_kwargs)
            else:
                kwargs = self.plane_kwargs.copy()
                kwargs.pop('m_xy')
                p = plane(**kwargs)
            p.write_points("./outputs/%s_test.data" % p.type_name)
...
class SmallWidthValuesTest(PlaneObjectsTest):
    ...
    def count_written_atoms(file_path):
        with open(file_path) as file_:
            ...
            # move down lines until atoms section
            line = file_.readline()
